...
<template>
  <div class="mv">
    ...
    <div class="mv-scroll">
      ...
      <!-- 品目リスト -->
      <div v-if="filteredItems.length" class="mv-list">
        <div v-for="item in filteredItems" :key="item" :class="['mv-item', { changed: _delta(item) !== 0 }]">
...
.mv-back { border: none; background: none; color: var(--primary, #2563eb); font-size: 14px; font-weight: 700; cursor: pointer; padding: 4px 2px; }
.mv-title { font-size: 16px; font-weight: 800; color: #1e293b; }
...
.mv-item { display: flex; align-items: center; gap: 10px; background: #fff; border: 1.5px solid #e2e8f0; border-radius: 12px; padding: 10px 12px; }
.mv-item.changed { border-color: #a7f3d0; background: #f0fdf9; }
