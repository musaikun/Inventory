...
// ── 手動非表示（左スワイプ）──────────────────────────────────────────────────
// 操作そのものは useRowHideSwipe が持つ（振り分けの品目一覧と同じ操作にするため）。
const {
  ...
  enabled: () => canSwipeHide.value,   // 読み取り専用・申請も不可な画面ではスワイプしない
  // ゲストは自分では隠せないので、ホストへの「申請」として送る
  onHide: item => emit(hideIsRequest.value ? 'request-hide' : 'hide-item', item),
})
