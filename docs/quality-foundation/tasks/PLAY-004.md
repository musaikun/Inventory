...
## 公開legal文面の実装整合(2026-08-02 / Claude Code / レビュー待ち)

Codex review指摘1（terms正本の未同期）と、`PLAY-002`のD-019実装で生じた文面の矛盾をまとめて解消した。
Worker・App実装は無変更で、legal文面と回帰testのみ。

- **terms正本を公開HTML側へ同期**（`docs/legal/terms.md`）。公開/landing termsは既に正しく、正本だけがずれていた。
  - 第6条3・第11条3: 「利用者へ通知します」→「本サービス上へ掲示します／お知らせします」。
    **本サービスはメールアドレス等の連絡先を保持しないため、個別通知は履行できない**（実装に合わせた文面）。
    将来、復旧用メールアドレスを保持する実装（`docs/proposals.md` 2026-07-28）を入れる場合は「通知」へ再改定する。
  - 第7条2/5: 「一切の責任を負いません」→「責任を負いません」。全部免責は消費者契約法8条で無効となり得るため。
  - 第1条「利用条件」、第2条「操作履歴等」、第8条3の表現を公開版へ揃え、改定日と改定理由を追記。
- **D-019の実装反映**: 端末ID・端末名・天気の位置情報とキャッシュを「削除しても端末に残る」→
  「**アカウント削除の完了時に端末から自動的に削除される**」へ改めた。端末IDが次回起動時に再発行される旨も明記。
  `privacy`（保持期間表・§8）、`support`（残るものの表・説明）、`landing`の両コピー、正本`docs/legal/*.md`の
  計6ファイルを同時更新。「残るもの」は入力方法などの表示設定のみになった。
- **回帰testを追加**（`legalPages.test.js`）:
  - 条文単位の同期チェック（終了通知・免責・規約変更・用語）を3コピーへ適用。旧文言の再発も禁止。
  - 端末固有データの自動削除の記述を privacy 3コピー＋support 2コピーへ適用し、
    旧記載（`端末ID・端末名・天気の位置情報は端末の設定として残ります`／「残るもの」表の端末ID行）を禁止。
- 検証: 文面を`git stash`した状態で**6件が失敗**することを確認（回帰として機能する）。
  `legalPages.test.js` 56 tests passed。App 58 files / 497 tests passed。App production build成功。
- **PLAY-003へ引き継ぎ**: `data-safety-form-draft.md` / `data-safety-audit.md` の端末データ保持の記述は
  未更新。Data Safety申告と公開文面の整合はCodexのPLAY-003で最終照合する。
...
- ~~**terms正本の同期（Codex review指摘1）**~~ → 2026-08-02に完了（下記「公開legal文面の実装整合」）。
