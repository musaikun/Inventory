...
**testが挙動を確認できていなかった範囲**だった。3点とも先に失敗するtestを追加している
（追加10件が修正前に失敗）。

### 1. P1: 取消必須の409で閉じると復旧手段を失う

前回の実装は `OUTCOME_UNKNOWN` だけを close 禁止にし、`mustCancel` では閉じられた。
「サーバー上の状態は確定していて履歴から辿れる」と判断したが、**これは誤り**だった。

- 親の `useDataImport.closeStocktake()` は close で `stocktakePlan` と `importBatchId` を捨てる。
- 履歴画面に別の取消導線が無い。
- したがって**閉じた時点で `DELETE /imports/:batchId` を二度と呼べない**。
  再試行では解消しないコードなので、取消が唯一の復旧手段だった。

修正:

- `closeBlocked = hasUnknown || mustCancelList.length > 0` を close の3経路すべてに効かせる。
  閉じるボタン・Escape・オーバーレイのいずれでも `close` を emit しない。
- 「サーバー側の記録が残っているため閉じられません。この取込を取り消してから閉じてください。」を出す。
- `mustCancelList` は取消成功（`undone`）で空になるので、取消後は閉じられる。取消失敗では閉じられない。
- 誤って「閉じられる」を期待していた既存testを、閉じられないことの確認へ置き換えた。

### 2. P1: 列数不一致が正常データとして受理される

`resultCsvParser` と `deliveryImportParser` にヘッダとの列数照合が無かった。
品目取込（`itemImport._collectRows`）だけが持っていて、他の2入口に無かった。

実測（ヘッダ `日付,品目名,単位,数量,単価` に `2026-01-01,米,1,100`）:

| | 修正前 | 修正後 |
|---|---|---|
| 単位 | `1` | 行エラー |
| 数量 | `100` | 行エラー |
| 納品取込の品目名 | 別列へずれる | 行エラー |
| errors | **空** | 1件 |

- 両parserへ `_colCountError()` を追加し、**値を読む前に**照合する
  （読んでからでは品目名すら別の列を指す）。
- エラーの形は品目取込と同じ `{ line, column: null, columnLabel: '列数', value: 'N列', reason }`。
- 列が多い行も同じく拒否する。
- `parseResultCSV`（確認画面なしの復元）は従来どおりフェイルクローズで1件も取り込まない。

### 3. P1: 日付空欄の実データ行が無通知で消える

`parseResultSnapshots` は日付が空欄なら、品目名・数量が入っていても黙って `continue` していた。
他に正常行が1件あれば取込全体は成功し、`errors` も空だった。

- **数量が入っている行＝実データ行**とみなし、日付が空欄でも行エラーとして出す。
  理由は「日付が空欄です（過去棚卸の取込には日付が必要です）」。
- 数量も空の行だけは集計行などの非データ行として従来どおり黙って飛ばす。
- 既存test「有効な日付行が無ければエラー」は入力が `,豚バラ,3` で、まさにこのケースだった。
  旧アサーション（`/見つかりません/`）はバグ挙動を固定していたので、
  行番号つきの理由を確認する形へ更新し、「日付も数量も空」の場合の従来動作を別testで残した。

### 検証（2026-08-19 / レビュー修正後）

| command | 結果 |
|---|---|
| 追加10 testの修正前 | **10 failed / 81 passed**（列数6件・日付空欄2件・close禁止3件のうち重複を除く） |
| 対象4 file（修正後） | **135 passed** |
...
- 過去棚卸取込のモーダルは、**結果不明に加えて「取消必須の409」でも閉じられない**。
  閉じられる条件は「全日確定」「HTTP失敗のみ（取消不要）」「取消成功」の3つ。
- 棚卸結果CSV・納品CSVは**ヘッダと列数が一致しない行を受理しない**。
  列数が揃った既存の正常CSVは影響を受けない（テンプレCSVを含めて回帰testで確認）。
- 過去棚卸取込で日付空欄の実データ行は、確認画面に行番号つきで出る。
...
- 画面に「再試行では解消しません。取り消してから取り込み直してください」を `role="alert"` で出し、
  サーバーが返した理由文をそのまま残す。
- ~~結果不明ではないので close は塞がない~~
  **2026-08-19 訂正（Codexレビュー指摘1）:** これは誤り。close で親が計画と `importBatchId` を
  捨て、履歴画面に別の取消導線が無いため、閉じると取消できなくなる。`mustCancel` も close 禁止にした
  （上記「Codexレビュー指摘3件の修正」1）。
