...
> **統合先（2026-08-01）**: `R-001` と `F-001`〜`F-004` は新規ID（`BUG-00x`）を作らず、
> 既存の [`DATA-002`](tasks/DATA-002.md) へ統合しました。`DATA-002` は P2 から **P1** へ変更しています。
> **状態の正本は [`task-list.md`](task-list.md)** で、このファイルは報告全文・コード根拠・
> 本番D1調査結果を保存する台帳として維持します（内容は削除しません）。
> 着手条件（Codexの作業完了後）は変わっていません。
...
| F-004 | 2026-07-28 | セッション削除時、ローカルに snapshot が無いとD1側が消えず孤児として残る | D1・分析 | P2 | サーバー修正はコード済み・本番未deploy。復活経路をv0.116.2で遮断・分析から削除可（詳細参照） |
...
- **2026-09-26 追記（User報告「カレンダーに無い日付の棚卸が在庫分析に出る」の調査）**:
  - サーバー側の一括削除（`handleSessionDelete` が store_history 等も同じ batch で消す）はコードに入っているが、
    **本番Workerは旧版のまま**（WEB-04 の migration 0009〜0017・Worker deploy 待ち）。develop preview も本番Workerを使うため、
    削除時に孤児ができる状態は本番で続いている。
  - **新たに判明した復活経路**: 起動時の送り直し（`historyBackfill.missingSnapshots`）が、サーバーが一度受け付けた記録（synced）も
    「リモートに無い」だけで送り直していた。別端末で削除した棚卸が、記録の残っている端末から store_history へ再作成され、
    `handleHistoryPost` は sessions の存在を確かめないので、そのまま孤児になる（修正版Workerでも起きる）。
  - **v0.116.2 で対処（アプリ側）**: ①synced 済みの記録は送り直さない（復活を止める。端末の記録は自動では消さない＝
    サーバーの履歴行だけ失われた場合に唯一の写しを消さないため）。②在庫分析に「カレンダーに無い棚卸」（sessions に行が無い記録）を
    出し、確認して削除できる（`services/historyOrphans.js`。sessions は50件上限なので、返った範囲より古い記録は判定しない）。
  - **未対処**: Worker 側で「存在しない/未完了セッションの history POST を拒否」（WEB-04 の deploy とまとめる）。F-002（50件上限）は、
    発注セッションも枠を使うため、古い棚卸がカレンダー側だけで見えなくなる別の食い違い原因として残る。
