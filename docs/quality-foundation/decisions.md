...
## D-025 — セッションはバージョンを触らない。ビルドの識別は commit SHA で行う

- 日付: 2026-08-27
- 状態: 一部を [D-026](#d-026--変更を-push-するたびにバージョンを上げるorigin-の値を起点にする)
  で置き換え（「実装セッションは触らない」の部分）。commit SHA を正とする点は引き続き有効
- 決定者: User
...
- 検討して**採らなかった**案: バージョンを6桁（`0.87.0.0.0.0`）にして桁ごとに担当を分ける。
  一度実装したが、**衝突を管理するだけで消していない**。周知と遵守が要り、
  semver から外れる（`npm version` が `Invalid version` で落ちる／将来の publish・
  semver前提ツールに渡せない）。桁は「枠Dの3回目」までしか示さず、
  結局あとでコミットへ手で対応づける作業が残る。
- 判断:
  - `app/package.json` の `version` は **3桁semver のまま**。上げるのは
    **User / PM セッションだけ**で、リリースの区切りに限る。
  - **実装セッションは version を一切触らない。** 書かないので衝突しない。
  - 「どのビルドか」は **commit SHA（短縮7桁）**が示す。ビルド時に `__BUILD_SHA__` へ
    埋め、ランディングと設定の「アプリ情報」に `v0.89.0 (207ec0d)` の形で出す。
- 番号より SHA が優れている点: **画面の表示から該当コミットを直接特定できる**。
  採番の周知も、上書き事故もない。semver のままなので将来の制約も作らない。
- SHA の取得順: `GITHUB_SHA` → `CF_PAGES_COMMIT_SHA` → `git rev-parse --short=7 HEAD`。
  git が無い環境（tarball ビルド等）では空になり、表示側が括弧ごと省く。
  `actions/checkout` は既定 `fetch-depth: 1` だが SHA は取れる
  （通算コミット数が要るなら `fetch-depth: 0` が必要 — 現状は使っていない）。
- D-023 との関係: 「変更ごとに更新する」は**実装セッションには適用しない**。
  変更の内容は commit message と [`session-log.md`](session-log.md) が示す。
  minor / patch の使い分けは、User / PM がリリース単位で上げるときにだけ効く。
...
## D-026 — 変更を push するたびにバージョンを上げる。origin の値を起点にする

- 日付: 2026-09-20
- 状態: 採用
- 決定者: User
- 背景: [D-025](#d-025--セッションはバージョンを触らないビルドの識別は-commit-sha-で行う) で
  実装セッションは version を触らないことにしていたが、Userから「変更の度に上げてほしい」
  と指示。リリースの区切りを待つと、画面の版だけでは**いつの変更まで入っているかが
  分からない**（SHAは分かるが、人が口頭でやりとりするのは版のほう）。
- 判断:
  - 変更を `develop` へ push するたびに `app/package.json` の `version` を上げる。
  - 上げ幅は **修正・微調整 = patch ／ まとまった機能・仕様変更 = minor**。
  - **上げる前に `git fetch origin develop` して、origin の値を見てから +1 する。**
- D-025 の衝突リスクは消えていない: 複数セッションが同時に上げると採番が衝突し、
  **値が同じだと merge で競合すらしない**（`v0.79.0` / `v0.80.0` / `v0.82.x` / `v0.86.0` で
  4回起きている）。origin を起点にするのは、そのうち「手元の古い値から+1して、
  同じ番号を名乗る」形だけを防ぐため。**同時に push すると依然として起こりうる**ので、
  複数セッションで並行して作業するときはここを見直すこと。
- ビルドの識別は引き続き commit SHA が正（D-025のこの部分は有効）。
  画面には `v0.97.1 (ff708a6)` の形で両方出る。
