...
### CCレビュー修正 第3セッション（取込のデータ品質 / 2026-08-10・IMPORT-001）

Codex の 2026-08-09 レビューで確認された取込のデータ欠陥を修正した。**レビュー待ち**で、
Codex承認前に完了・公開可としない。詳細は
[`quality-foundation/tasks/IMPORT-001.md`](quality-foundation/tasks/IMPORT-001.md)。

- **CSVの読み方を1本にした**（`app/src/utils/csvParse.js` 新規）。品目取込・棚卸結果取込・
  納品取込が同じ欠陥を持つ1行パーサを3本持っていた。`"1,200"` が **`1`** になり、
  `5"" 皿` が **`5 皿`** になり、閉じていない引用符が黙って通っていた。
- **不正な数値を「既存値を維持」にすり替えない。** その行をエラーにし、行番号・列・理由を
  確認画面に出す。列数不一致・ヘッダ無しファイル（先頭品目が消えていた）も同じ扱いにした。
- **エイリアスを黙って奪わない。** 既存品目の別名を新しい品目が上書きできていた。
  既定では奪わず、衝突を確認画面に出して「既存を優先／ファイルを優先」を選ぶまで取り込めない。
- **確認画面と実際の取込が同じ計画データを使う。** 以前は解析と計画を2回組み直していた。
  分類コード・軸名・品目名の切り詰め・発注点の扱いも確認画面へ追加した。
- **過去棚卸取込を通常の棚卸と同じ session として保存する。** 日付キーの直接書き込みをやめ、
  取込前プレビュー → サーバー保存の確認 → 端末反映の順にした。同じ日に棚卸があっても
  既定では消さず、取込は `importBatchId` 単位でまとめて取り消せる。
  実装: `worker/src/pastImport.js`、`app/src/services/pastImportPlan.js`、
...
### CC第2セッション（品目マスタ取込の本修理 / 2026-08-08）

- **取込の既定が「追加・更新」になった**。CSV・Excel・PDFのどれで取り込んでも、
  ファイルに載っていない既存品目・単価・別名・分類は消えない。同名品目は列単位で上書きし、
  空欄の列は既存値を残す。従来の全置換は確認画面で「全入れ替え」を明示選択したときだけ。
- **取込前プレビューを全経路に追加**。追加 / 更新 / 変更なし / 除外（＋全入れ替え時は削除）の
  件数、取込後の総件数、更新品目のフィールド単位の差分、除外行の行番号と理由を確定前に表示する。
  無料プラン上限による切り捨ても取込前に警告する（従来は無言だった）。
- **取込直後に限り1回だけ取り消せる**（メモリ上の退避。再読込・アカウント切替・ホスト設定受信・
  取込以外の品目変更で失効）。恒久的なスナップショット機構は作っていない。
- PDF取込をβ表記にした。推奨フォーマット（品目リスト出力のCSV）は往復可能になり、
  並び替えの分類先・発注点まで復元される。
- 実装: `app/src/utils/itemImport.js`（解析→計画→適用の純粋関数）、
  `app/src/components/ItemImportPreviewModal.vue`。Worker / D1 / migration は無変更。
- 検証: S1・S2・S7・S8 と統合後で App 70 files / 604 tests passed、production build 成功、
  Worker 15 files / 196 tests passed。
- 未実施: 実機UI確認（375px・デスクトップ）。設計判断は [`proposals.md`](proposals.md) でPM判断待ち。
...
- **在庫分析ダッシュボード**（ManagerDashboard：在庫金額・前回差・ABC・曜日・異常検知）
...
- **振り分けの1画面化**（2026-09-02 / AxisAssignFocus）。分類先を縦の3Dホイールにして
  「選ぶ面 ⇄ 品目一覧」の往復を無くし、振り分け先を常に画面へ出す。追加・削除はホイール隣の
  レール、名前変更と順番は ⚙ の管理ページ（ドラッグ＋FLIP）。品目一覧の仕様（ジャンル別
  アコーディオン・絞り込み・左スワイプ非表示）は従来どおり。設計判断は `proposals.md`
  2026-09-02 のエントリで PM トリアージ待ち。2026-09-03に先頭・末尾を両方向へつなぐ
  循環動作、ホイールと品目一覧の同期した面積遷移、上下の並べ替えFLIPを追加。
  2026-09-04の実機フィードバックで、スマホの直近120msから慣性速度を推定し、開閉を700msへ調整。
  並べ替えのPointer Captureは固定された一覧へ移し、1回のドラッグで複数段を移動可能にした。
  ホイール内の文字選択・長押しメニュー・要素ドラッグも抑止し、回転操作の中断を防いだ。
  さらに、帯へ畳んだままでも中央の件数を押して振り分け済みを開けるようにし（押しても広げない）、
  押下は`pointerup`でも成立させた。面積は「帯」と「全体」の2段だけにし、固定196pxの中間段と
  回し終わり900msの自動縮小を廃止した。分類先が0件のときは、ホイールの場所に
  「まだありません／＋で作ってください」を出す。
- **振り分けの逆向き導線**（2026-09-14 / AxisAssignFocus）。品目一覧の行を**長押し**すると、
  その品目の分類先を選ぶシートが押した行の近くに開く。既定の「分類先を固定して品目を連打」は
  そのまま残し、分類先がばらばらな品目が続くときにホイールを回し直す往復だけを消す。
  シートの並びは「いま入っている → 同じジャンルの品目が集まっている → 直近に使った →
  ホイール順」。ドラッグではなくタップにした理由（速さ・スクロールとの競合・20件の分類先）と
  未決事項（複数所属の頻度、「外す」の設計）は `proposals.md` 2026-09-14 のエントリ。
  config / localStorage / D1 / Worker は無変更。実機確認は未実施。
- **データ管理・棚卸一覧のUI整理**（2026-09-15 / User指示12件）。取り込む・書き出すの入口を
  1つずつにして押してから種類を選ぶ形にし、品目一覧を畳まず常設して発注点・補充目標・
  表示/非表示をその場で設定できるようにした。「分類の追加」は「並び順設定」へ改称し、
  ジャンルを取込元由来のグループとして自作と同じ列に並べた。実装として死んでいた
  自動非表示のUIと、「前回までに入力した品目のみ表示」の絞り込みを機能ごと削除。
  音声入力は検索欄の隣の小さいボタンへ、読み込み中は共通の`LoadingSpinner`で動きを出す。
  分類先の並べ替えのつまみは220ms持ってから掴む。設計判断と未決事項は
  `proposals.md` 2026-09-15 のエントリ。config / D1 / Worker は無変更。実機確認は未実施。
