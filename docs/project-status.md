...
- **入出庫の D1 永続化** — Wave 2.5 #2 完了（`0010_movements`・冪等POST＋開始時ロード・
  再送キュー・削除もD1連動）。⚠️ R5-01（upsertのテナント条件）修正が完了条件 → `intake-reviews.md`
- **論理出庫（消費量の逆算）** — 発注点の目安を消費ベースへ。
- **日別メモ**（内部イベント・学習除外）⚠️ R5-02: アカウント切替消去に未配線。
- 天気の地名表示（BigDataCloud 逆ジオ）⚠️ 外部送信先2つ目（R4-01の対象拡大）。
...
- **天気連携（P3）** — Open-Meteo（鍵不要・オプトイン位置取得・1hキャッシュ）で
  カレンダーに気温/降水/天気を表示。⚠️ ポリシーへの位置情報記載が本番前必須（R4-01）。
