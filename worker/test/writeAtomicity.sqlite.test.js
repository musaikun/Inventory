...
function snapFor(inventory, { sessionId = SESS, date = '2026-08-09', extra = [] } = {}) {
  return {
    date, sessionId,
    items: [
...
describe('棚卸完了 — 実SQLiteでの原子性と冪等性', () => {
  ...
  it('品目が減った再送では前回ぶんが残らない', async () => {
    ...
    await handleSessionComplete(h.db, SHOP, SESS, { inventory: { 牛乳: { qty: 1 } }, prices: {}, snapshot: snapFor({ 牛乳: { qty: 1 } }) })
...
describe('snapshot を完了と同じトランザクションで書く（DATA-001 / F-001）', () => {
  ...
  const snap = (sessionId, date, inventory = { 牛乳: { qty: 1 } }) =>
  ...
  it('途中で落ちたら snapshot も残らない（部分成功を作らない）', async () => {
    ...
    h.failBatchAt(2)
    const res = await handleSessionComplete(h.db, SHOP, SESS, {
      inventory: makeInventory(30), prices: {}, snapshot: snap(SESS, '2026-08-09', makeInventory(30)),
    })
    expect(res._status).toBe(503)
    expect(h.rows('SELECT * FROM store_history')).toHaveLength(0)
    expect(h.rows('SELECT * FROM inventory_lines')).toHaveLength(0)
  ...
  it('同じ日の2回目の棚卸が1回目のスナップショットを上書きしない（F-001）', async () => {
    ...
    await handleSessionComplete(h.db, SHOP, SESS2, {
      inventory: { 砂糖: { qty: 5 } }, prices: {}, snapshot: snap(SESS2, '2026-08-09', { 砂糖: { qty: 5 } }),
    })
  ...
  it.each([0, 1, 2, 3])('batch の %i 番目で落ちても全体が巻き戻る', async (failIndex) => {
    ...
    const res = await handleSessionComplete(h.db, SHOP, SESS, {
      inventory: makeInventory(25), prices: {}, snapshot: snap(SESS, '2026-08-09', makeInventory(25)),
    })
    expect(res._status).toBe(503)
    expect(res.retryable).toBe(true)
    expect(h.rows('SELECT * FROM inventory_lines')).toHaveLength(0)
...
describe('履歴の server revision（第2セッション §2）', () => {
  // inventory と一致する snapshot（DATA-002 §1）。既定は 牛乳 1 件。
  const snap = (sessionId, date = '2026-08-09', inventory = { 牛乳: { qty: 1 } }) =>
    snapFor(inventory, { sessionId, date })
  ...
  it('別セッションを保存しても、既存行の revision は下がらない', async () => {
    ...
    const a = await handleSessionComplete(h.db, SHOP, SESS,  { inventory: { A: { qty: 1 } }, prices: {}, snapshot: snap(SESS, '2026-08-09', { A: { qty: 1 } }) })
    const b = await handleSessionComplete(h.db, SHOP, SESS2, { inventory: { B: { qty: 1 } }, prices: {}, snapshot: snap(SESS2, '2026-08-10', { B: { qty: 1 } }) })
